% rwa_figure2_supp.m
%
% Generate the base image that is similar to Figure 2 of the manuscript,
% except we are using ASAQ as the point of comparision instead of AL5
%
% Requires the following files be present in the 'data' directory:
%
% rwa-AL3-parsed.csv        - AL, three day course
% rwa-ASAQ-parsed.csv       - ASAQ
% rwa-DHAPPQ-parsed.csv     - DHA-PPQ
% rwa-MFT-parsed.csv        - MFT with 75% ASAQ and 25% DHA-PPQ
% rwa-ALAQ-parsed.csv       - ALAQ, triple-ACT
% rwa-ROTATION-parsed.csv   - DHA-PPQ for three years, 
%                             then MFT of AL (five day) and ASAQ
%
% Relevent files are created by the rwa_plot_genotype.py script and are
% placed by the organize.sh script.
clear;
...
ASAQ=csvread('./data/rwa-ASAQ-parsed.csv',1,0);
nr = size(AL3,1);
...
red2 = [230 85 13]/255; % this is the most probably intervention, or ASAQ
red3 = [253 141 60]/255; % these are alternatives to ASAQ

blue1 = [8 81 156]/255; % this is the status quo, or AL3
blue2 = [49 130 189]/255; % this is the most probably intervention, or ASAQ
blue3 = [107 174 214]/255; % these are alternatives to ASAQ

gray1 = [37 37 37]/255; % this is the status quo, or AL3
gray2 = [99 99 99]/255; % this is the most probably intervention, or ASAQ
gray3 = [150 150 150]/255; % these are alternatives to ASAQ
...
xx = transpose(ASAQ(:,1));
yy2 = transpose(ASAQ(:,7));
yy1 = transpose(ASAQ(:,5));
...
plot( ASAQ(:,1), ASAQ(:,6), 'Color', gray2, 'LineStyle', '-', 'Marker', 'none' );
...
xx = transpose(ASAQ(:,1));
yy2 = transpose(ASAQ(:,10));
yy1 = transpose(ASAQ(:,8));
...
plot( ASAQ(:,1), ASAQ(:,9), 'Color', blue2 , 'LineWidth', lw , 'LineStyle', '-' , 'Marker', 'none' ); 
...
xx = transpose(ASAQ(:,1));
yy2 = transpose(ASAQ(:,4));
yy1 = transpose(ASAQ(:,2));
...
plot( ASAQ(:,1), ASAQ(:,3), 'Color', red2 , 'LineWidth', lw, 'LineStyle', '-' );
...
for spi=2:6
    ...
    if spi==6
        C = csvread('./data/rwa-AL-THEN-DHAPPQ-789-parsed.csv',1,0);
        bottom = true;
    end
    C = C(trm:nr-trm+1,:);
    ...
    plot( ASAQ(:,1), ASAQ(:,3), 'Color', red2 , 'LineWidth', lw, 'LineStyle', '-', 'Marker', 'none' );
